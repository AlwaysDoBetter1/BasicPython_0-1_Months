...
for i in range(n):  # selection sorting all elements of list
    mx_ind = n - 1 - i  # for replace elements from end to start with step: -1
    for j in range(n - i):
        if a[j] > a[mx_ind]:  # finding max element
            mx_ind = j

    a[n - 1 - i], a[mx_ind] = a[mx_ind], a[n - 1 - i]  # swap the maximum element with the last not replaced element
